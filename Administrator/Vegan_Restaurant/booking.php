...
function deleteAllBookings($conn) {
    $sql = "DELETE FROM bookings WHERE username IN (SELECT username FROM users WHERE store = 'Vegetarian')"; // Modify the query to delete bookings only from the American store
    if ($conn->query($sql) === TRUE) {
        echo "All bookings from the Vegetarian store deleted successfully";
    } else {
...
$sql = "SELECT b.* FROM bookings b JOIN users u ON b.username = u.username WHERE u.store = 'Vegetarian'";
...
if ($result->num_rows > 0) {
    ...
    echo "No bookings from the Vegetarian store found";
}
